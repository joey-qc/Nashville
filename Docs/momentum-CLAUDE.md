...
- All migrations live in `Momentum.Data/Migrations/`
...
These colors are the single source of truth for category representation throughout the entire application. Apply them consistently in charts, chips, badges, checkboxes, and any other UI element that represents a category.

| Category | Color | Suggested Hex |
|---|---|---|
...
- Category colors must be defined as constants or a static dictionary in a shared location (e.g., `Momentum.Shared/Constants/CategoryColors.cs`) and referenced everywhere — never hardcoded inline in individual components.
...
- The cache is invalidated and refreshed on any CRUD operation in the Manage Activities screen.
...
When a new user registers, the following activities must be automatically seeded into their activity library. This is triggered in the registration flow, not via EF Core seed data.

| Activity Name | Categories | Default Points |
|---|---|---|
| Exercise / Gym | Physical | +20 |
| Hiking (Solo) | Physical | +20 |
| Hiking (With Others) | Physical, Social | +20 |
| Meditation | Mental, Spiritual | +20 |
| Reading (Nonfiction) | Mental | +20 |
| Journaling | Mental, Spiritual | +20 |
| Cooking a Healthy Meal | Physical, Housekeeping | +20 |
| Cleaning / Organizing | Housekeeping | +20 |
| Socializing with Friends | Social | +20 |
| Calling Family | Social | +20 |
| Travel (Solo) | Mental, Spiritual | +20 |
| Travel (With Others) | Mental, Spiritual, Social | +20 |
| Watching Excessive TV | Mental | +20 |
| Skipping Sleep | Physical, Mental | +20 |
...
*Version 1.0 — Read this file before every code generation or modification task*
